...
import { col, data, label } from "motion/react-client";
...
function SalesChart({ bookings, numDays }) {
	...
	const data = eachDayOfInterval({
		...
		return {
			...
			...bookings.reduce(
				(total, booking) => {
					if (!isSameDay(booking.created_at, date)) return total;
					total.totalSales += booking.totalPrice;
					total.extrasSales += booking.extrasPrice;
	...
	return (
		<StyledSalesChart>
			<Heading as={"h2"}>
				Sales from {`${data[0].label} to ${data[data.length - 1].label}`}
			</Heading>
